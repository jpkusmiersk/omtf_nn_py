...
class LutInterLayer(keras.layers.Layer):
    def __init__(self, name, 
        ...
        if input_offset == None :
            self.input_offset = (lut_size-1)/2.
        else :    
        ...
        print("constructing LutInterLayer ", name, "lut_size", lut_size, "num_inputs", num_inputs, "num_outputs", num_outputs, "input_offset", input_offset, "self.input_offset", self.input_offset, "last_input_is_bias", self.last_input_is_bias)
    ...
    def plot_luts(self, path="") :
        ...
        nRows =  self.num_inputs
        if nRows > nRows :
            nRows = 8*4
        
        nColumns = 4    
        #nColumns = self.num_outputs
        #if nColumns > 4 :
        #    nColumns =  4
        #                       (y,x)                                                  # x y    
        fig, ax = plt.subplots(nrows = nRows, ncols = nColumns, sharex=True, figsize=(18, nRows *4)) 
        ...
        i_in_offset = 0
        # trick to see the luts in the middle of the of th lut set, e.g. for mnist, where the is over 700 inputs, TODO: tune the numbers  
        ...
        for i_out in range(0, min(ax.shape[1], self.num_outputs)) :
            for i_in in range(0, min(ax.shape[0], self.num_inputs)) :
                ...
                print("LutInterLayer.plot_luts(): created", ax[i_in, i_out].get_title())
        
        plt.savefig(path + self.name + "_luts.png", bbox_inches="tight")
        print("LutInterLayer.plot_luts(): created", path + self.name + "_luts.png")
        return fig        
...
class LutInitializerLinear(tf.keras.initializers.Initializer):
    ...
    def __init__(self, maxLutVal, initSlopeMin, initSlopeMax, lutRangesCnt, reLu = False):
    ...
    def __call__(self, shape, dtype=None) :
        ...
        for i_out in range(0, num_outputs) :
            for i_in in range(0, num_inputs) :
                ...
                b0 = -b1 * (lutSize - 1) / self.lutRangesCnt / 2.;
                offset = b0;
                if offset * b1 > 0 :
                    offset = -offset
                ...
                for iAddr  in range(0, lutSize) :
                    ...
                    #if iAddr == lutSize -1 :
                    #    luts_float[i_in][iAddr][i_out] = 0 #TODO if last_input_is_bias and the last addres is for the no_hit then it is better to have 0 here 
                    ...
                    if self.reLu :
                        if s2 == 0 and luts_float[i_in][iAddr][i_out] > 0 :
                            luts_float[i_in][iAddr][i_out] = 0
                        elif s2 == 1 and luts_float[i_in][iAddr][i_out] < 0 :
                            luts_float[i_in][iAddr][i_out] = 0        
