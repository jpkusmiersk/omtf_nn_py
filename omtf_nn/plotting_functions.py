...
def plotPtGenPtRec(df, plotDir, oneOverPt=False) :
    ...
    ax1[0,1].set_xlabel(r'$p_{T}^{GEN}$')
    ax1[0,1].set_ylabel(r'$p_{T}^{NN}$')
    
    ax1[1, 1].hist2d(x = df["GEN_pt"], y = df["OMTF_pt"], weights=df["weight"], 
                     bins=(50, 50), range=[[0, ptMax], [0, 200]], cmap=plt.cm.jet, norm=colors.LogNorm())
    ax1[1,1].set_xlabel(r'$p_{T}^{GEN}$')
    ax1[1,1].set_ylabel(r'$p_{T}^{OMTF}$')
    
    binWidth = 2   
    df["delta"] = df["NN_pt"] - df["GEN_pt"]
    df["abs_delta"] = np.abs(df["NN_pt"] - df["GEN_pt"])
    df["delta_squared"] = (df["NN_pt"] - df["GEN_pt"])**2
    df_grouped = df.groupby(by=binWidth*(df["GEN_pt"]/binWidth).astype(int))
    
    binCenters =  np.array(list(df_grouped["delta"].groups.keys())) + binWidth/2.0 
    
    mean_y_pulls = df_grouped["delta"].mean()/binCenters 
    mean_y_abs_pulls = df_grouped["abs_delta"].mean()/binCenters 
    mean_y_squared_pulls = np.sqrt(df_grouped["delta_squared"].mean())/binCenters 
